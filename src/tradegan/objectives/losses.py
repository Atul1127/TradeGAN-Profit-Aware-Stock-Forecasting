...
def _grad_norm(module):
    ...
    for p in module.parameters():
        if p.grad is not None:
            total = total + p.grad.detach().norm(2) ** 2
    return total.sqrt()


def _safe_ratio(num, den):
    return num / den.clamp_min(torch.finfo(num.dtype).eps)


def GradientCheck(ticker, gen, disc, gen_opt, disc_opt, criterion, n_epochs, train_data,
    ...
    norms = [[] for _ in range(5)]
    for _ in range(n_epochs):
        shuffled = train_data[torch.randperm(n, device=train_data.device)]
        for start in range(0, n, batch_size):
            batch = shuffled[start:start + batch_size]; cur = batch.shape[0]
            c = batch[:, :l].unsqueeze(0).to(device=device, dtype=torch.float)
            r = batch[:, l:l + pred].unsqueeze(0).to(device=device, dtype=torch.float)
            h0d = torch.zeros((1, cur, hid_d), device=device); c0d = torch.zeros_like(h0d)
            h0g = torch.zeros((1, cur, hid_g), device=device); c0g = torch.zeros_like(h0g)
            disc_opt.zero_grad(set_to_none=True)
            fake = gen(torch.randn((1, cur, z_dim), device=device), c, h0g, c0g)
            fp = disc(torch.cat((c, fake.detach()), -1)); rp = disc(torch.cat((c, r), -1), h0d, c0d)
            dloss = (criterion(fp, torch.zeros_like(fp)) + criterion(rp, torch.ones_like(rp))) / 2
            dloss.backward(); disc_opt.step()
            fake = gen(torch.randn((1, cur, z_dim), device=device), c, h0g, c0g)
            fp = disc(torch.cat((c, fake), -1), h0d, c0d)
            p = fake.squeeze(0).squeeze(-1); t = r.squeeze(0).squeeze(-1); pnl = torch.tanh(tanh_coeff * p) * t
            values = [criterion(fp, torch.ones_like(fp)), pnl.mean(), torch.norm(p-t)**2/cur, pnl.mean()/pnl.std().clamp_min(torch.finfo(pnl.dtype).eps), pnl.std()]
            for i, value in enumerate(values):
                gen_opt.zero_grad(set_to_none=True); value.backward(retain_graph=True); norms[i].append(_grad_norm(gen).detach())
            gen_opt.step()
    if not norms[0]:
        z=torch.ones((), device=device); return gen,disc,gen_opt,disc_opt,z,z,z,z
    B,P,M,S,D=map(torch.stack,norms)
    return gen,disc,gen_opt,disc_opt,_safe_ratio(B,P).mean(),_safe_ratio(B,M).mean(),_safe_ratio(B,S).mean(),_safe_ratio(B,D).mean()


def GradientCheckLSTM(ticker, gen, gen_opt, n_epochs, train_data, batch_size, hid_d, hid_g, z_dim,
                      lr_d=0.0001, lr_g=0.0001, h=1, l=10, pred=1, diter=1,
    ...
    for _ in range(n_epochs):
        ...
        for start in range(0,n,batch_size):
            batch=shuffled[start:start+batch_size]; cur=batch.shape[0]
            c=batch[:,:l].unsqueeze(0).to(device=device,dtype=torch.float); r=batch[:,l:l+pred].unsqueeze(0).to(device=device,dtype=torch.float)
            h0=torch.zeros((1,cur,hid_g),device=device); f=gen(c,h0,torch.zeros_like(h0)); p=f.squeeze(0).squeeze(-1); t=r.squeeze(0).squeeze(-1); ps=torch.tanh(tanh_coeff*p)*t
            values=[ps.mean(),torch.norm(p-t)**2/cur,ps.mean()/ps.std().clamp_min(torch.finfo(ps.dtype).eps),ps.std()]
            for i,v in enumerate(values):
                gen_opt.zero_grad(set_to_none=True); v.backward(retain_graph=True); norms[i].append(_grad_norm(gen).detach())
            gen_opt.step()
    if not norms[0]:
        z=torch.ones((),device=device); return gen,gen_opt,z,z*0,z,z
    P,M,S,D=map(torch.stack,norms)
    return gen,gen_opt,_safe_ratio(M,P).mean(),torch.zeros((),device=device),_safe_ratio(M,S).mean(),_safe_ratio(M,D).mean()
